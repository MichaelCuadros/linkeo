...
// const STORAGE_KEY = "social_tiles_theme_v7";
...
//   width:${p=>p.$w}px;
//   border-radius:36px; padding:12px; background:#e5e7eb;
//   box-shadow: 0 12px 35px rgba(0,0,0,.15); border: 1px solid #d1d5db;
//   max-height: 85vh;
//   overflow: hidden;
// `;
...
//   ` : `background:${p.$bgCss || "#fff"};`}

//   box-shadow: inset 0 0 0 1px rgba(255,255,255,.08);
//   display:grid; gap:${p=>p.$gap}px;
...
//     ${p=>p.$avatarAlign==='left'?'flex-start':p.$avatarAlign==='right'?'flex-end':'center'};
//   img{ width:96px; height:96px; border-radius:999px; object-fit:cover; border:3px solid rgba(0,0,0,.06); }
// `;
// const Heading = styled.div`
//   text-align:${p=>p.$align};
//   h2{ margin:8px 0 4px 0; font-size:24px; letter-spacing:.2px }
...
//   border:1px solid #e2e8f0; background:#f8fafc; color:#111827;
//   &:focus{ outline:none; box-shadow:0 0 0 3px rgba(14,165,233,.25); border-color:#38bdf8; background:#fff; }
// `;
// const TextArea = styled.textarea`
//   width:100%; padding:12px 14px; border-radius:14px;
//   border:1px solid #e2e8f0; background:#f8fafc; color:#111827; min-height:70px; resize:vertical;
//   &:focus{ outline:none; box-shadow:0 0 0 3px rgba(14,165,233,.25); border-color:#38bdf8; background:#fff; }
// `;
...
//   box-shadow: 0 6px 22px rgba(0,0,0,.06); margin-bottom:14px;
// `;
// const StepHeader = styled.button`
//   all:unset; display:flex; align-items:center; justify-content:space-between; gap:12px;
//   width:100%; padding:14px 28px 14px 16px; /* ← más padding derecho */
//   cursor:pointer; background:#f8fafc; border-bottom:1px solid #eef2f7;
//   h3{margin:0; display:flex; align-items:center; gap:10px; font-size:16px}
//   > span{
//     white-space: nowrap;         /* evita cortes en móvil */
//     margin-right: 22px;          /* ← empuja un poco a la izquierda */
//   }
// `;
// const Badge = styled.span`
//   display:inline-flex; align-items:center; justify-content:center;
//   width:26px; height:26px; border-radius:999px; background:#111827; color:#fff; font-weight:700; font-size:12px;
// `;
...
// /* ====== Filas de control con valor visible ====== */
...
//   justify-self:end; min-width:52px;
...
//   column-count: 1;
//   column-gap: 12px;
//   @media (min-width:700px){ column-count: 2; }
// `;
...
//   padding:14px 28px 14px 16px; /* ← más padding derecho */
//   width:100%; cursor:pointer; font-weight:600; color:#fff;
//   background: ${({$brand}) => `linear-gradient(135deg, ${$brand} , ${$brand}cc)`};
// `;
// const TitleSpan = styled.span`display:inline-flex; align-items:center; gap:10px; font-size:16px;`;
// const Toggle = styled.span`
//   background: rgba(255,255,255,.18); padding: 6px 12px; border-radius: 999px; font-size:12px;
//   margin-right: 25px; /* ← lo empuja un poco a la izquierda */
// `;
// const CardBody = styled.div` padding:14px 14px 16px; border-top: 1px solid #e5e7eb; `;
// const Row = styled.div`
//   display:flex; gap:8px; align-items:center; margin-bottom:10px;
//   label{ display:flex; align-items:center; gap:6px; font-size:14px; color:#374151; }
// `;
// const Error = styled.div`color:#dc2626; font-size:12px; margin-top:6px;`;
...
//   // Identidad
//   title: "",                        // ← vacío para que funcione el placeholder
//   description: "",                  // ← vacío para que funcione el placeholder
//   align: "center",
//   textColor: "#FFFFFF",
//   avatarAlign: "center",
...
//   }, []);
//   useEffect(() => {
//     const data = { links: items.map(({ key, url, visible }) => ({ key, url, visible })), profile };
...
//     else { const l = document.createElement("link"); l.id=id; l.rel="stylesheet"; l.href=href; document.head.appendChild(l); }
//   }, [profile.fontFamily]);

//   // helpers
//   const byKey = (k) => items.find(i => i.key === k);
//   const toggleOpen = (k) => setItems(prev => {
//     const isOpen = prev.find(i => i.key === k)?.open;
//     return prev.map(i => i.key === k ? { ...i, open: !isOpen } : { ...i, open: false });
//   });
...
//     () => items.filter(i => i.visible && isValidUrl(i.url)).map(i => ({ ...i, href: normalizeUrl(i.url) })),
//     [items]
//   );
...
//     setErrors(e => ({ ...e, [k]: ok ? "" : "URL inválida (usa https://, mailto:, o tel:)" }));
//   };

//   // assets
//   const onAvatarFile = (file) => { const r = new FileReader(); r.onload = () => setProfile(p => ({ ...p, avatarDataUrl: r.result })); r.readAsDataURL(file); };
//   const onBgImageFile = (file) => { const r = new FileReader(); r.onload = () => setProfile(p => ({ ...p, bgImageDataUrl: r.result, bgMode: "image" })); r.readAsDataURL(file); };
//   const onPdfFile = (file) => { const r = new FileReader(); r.onload = () => setProfile(p => ({ ...p, pdfDataUrl: r.result, pdfName: file.name })); r.readAsDataURL(file); };
...
//   const buildVCard = () => {
//     const p = profile;
//     const socialNote = visibleLinks.map(l=>{
...
//         `ADR;TYPE=HOME:;;${escapeV(p.contactStreet)};${escapeV(p.contactCity)};${escapeV(p.contactRegion)};${escapeV(p.contactPostalCode)};${escapeV(p.contactCountry)}` : null,
...
//     const a = document.createElement("a"); a.href = URL.createObjectURL(blob); a.download = "perfil_y_links.json"; a.click(); URL.revokeObjectURL(a.href);
//   };
...
//           return { key: p.key, url: f?.url || "", visible: f?.visible ?? true, open: false };
//         });
//         setItems(mapped);
...
//           </label>
//           <Btn type="button" onClick={exportJson}>Exportar JSON</Btn>
//         </Actions>
//       </Header>

//       <Grid>
//         {/* PREVIEW (abajo en móvil) */}
//         <PhoneCol>
...
//                 <Avatar $avatarAlign={profile.avatarAlign}><img src={profile.avatarDataUrl} alt="Avatar" /></Avatar>
//               )}
//               <Heading $align={profile.align}>
//                 <h2 style={{opacity: profile.title ? 1 : .7, fontStyle: profile.title ? "normal" : "italic"}}>{titleText}</h2>
//                 <p  style={{opacity: profile.description ? 1 : .7, fontStyle: profile.description ? "normal" : "italic"}}>{descText}</p>
//               </Heading>

//               {visibleLinks.length === 0 && (
//                 <div style={{ color:"#e5e7eb" }}>Agrega enlaces para verlos aquí…</div>
//               )}
...
//                 const bgColor = profile.btnVariant==='filled' ? bg : (profile.btnVariant==='glass' ? 'rgba(255,255,255,.2)' : 'transparent');
...
//               {profile.pdfDataUrl && (
//                 <a href={profile.pdfDataUrl} target="_blank" rel="noreferrer" style={{ textDecoration:"none" }}>
//                   <Btn type="button">📄 Ver PDF</Btn>
//                 </a>
//               )}

//               {/* Guardar contacto vCard */}
//               <Btn type="button" onClick={downloadVCard}>📇 Guardar contacto (.vcf)</Btn>
//             </Preview>
//           </Phone>
//         </PhoneCol>
...
//               <h3><Badge>1</Badge> Perfil (avatar, nombre, descripción)</h3>
...
//                   <input type="file" accept="image/*" onChange={(e)=> e.target.files?.[0] && onAvatarFile(e.target.files[0])}/>
//                   <span> Alineación</span>
//                   <div className="seg">
//                     {["left","center","right"].map(v=>(
//                       <button key={v} className={profile.avatarAlign===v?"active":""}
//                         onClick={()=> setProfile(p=>({ ...p, avatarAlign:v }))}>
...
//                 <TextInput type="text" placeholder="Tu nombre o marca"
//                   value={profile.title} onChange={e=>setProfile(p=>({ ...p, title:e.target.value }))}/>
//                 <TextArea placeholder="Escribe una breve descripción…"
//                   value={profile.description} onChange={e=>setProfile(p=>({ ...p, description:e.target.value }))}/>

//                 <div className="row">
//                   <span style={{ fontSize:14, color:"#6b7280" }}>Alineación del texto:</span>
//                   <div className="seg">
//                     {["left","center","right"].map(al=>(
//                       <button key={al} className={profile.align===al ? "active":""}
...
//                          onChange={e=>setProfile(p=>({ ...p, textColor:e.target.value }))}/>
//                 </div>
//               </StepBody>
//             )}
//           </Step>
...
//               <h3><Badge>2</Badge> Fondo (color, degradado o imagen)</h3>
...
//                       <button key={m} className={profile.bgMode===m ? "active":""}
...
//                     <>
//                       <label>Color A</label>
//                       <input type="color" value={profile.bgColor}
//                              onChange={e=>setProfile(p=>({ ...p, bgColor:e.target.value }))}/>
//                       {profile.bgMode==="gradient" && (
//                         <>
//                           <label>Color B</label>
//                           <input type="color" value={profile.bgColor2}
//                                  onChange={e=>setProfile(p=>({ ...p, bgColor2:e.target.value }))}/>
//                           <label>Ángulo</label>
//                           <input type="range" min="0" max="360" value={profile.bgAngle}
//                                  onChange={e=>setProfile(p=>({ ...p, bgAngle:Number(e.target.value) }))}/>
//                         </>
//                       )}
//                     </>
//                   ) : (
//                     <>
//                       <label>Imagen</label>
//                       <input type="file" accept="image/*" onChange={(e)=> e.target.files?.[0] && onBgImageFile(e.target.files[0])}/>
//                       <div className="row" style={{width:"100%"}}>
//                         <div style={{flex:1, minWidth:200}}>
//                           <small>Overlay</small>
//                           <input style={{width:"100%"}} type="range" min="0" max="0.9" step="0.05" value={profile.overlayOpacity}
//                                  onChange={e=>setProfile(p=>({ ...p, overlayOpacity:Number(e.target.value) }))}/>
//                         </div>
//                         <div style={{flex:1, minWidth:200}}>
//                           <small>Pos. X ({profile.bgPosX}%)</small>
//                           <input style={{width:"100%"}} type="range" min="0" max="100" value={profile.bgPosX}
//                                  onChange={e=>setProfile(p=>({ ...p, bgPosX:Number(e.target.value) }))}/>
//                         </div>
//                         <div style={{flex:1, minWidth:200}}>
//                           <small>Pos. Y ({profile.bgPosY}%)</small>
//                           <input style={{width:"100%"}} type="range" min="0" max="100" value={profile.bgPosY}
//                                  onChange={e=>setProfile(p=>({ ...p, bgPosY:Number(e.target.value) }))}/>
//                         </div>
//                         <div style={{flex:1, minWidth:200}}>
//                           <small>Zoom ({profile.bgZoom}%)</small>
//                           <input style={{width:"100%"}} type="range" min="80" max="200" value={profile.bgZoom}
//                                  onChange={e=>setProfile(p=>({ ...p, bgZoom:Number(e.target.value) }))}/>
//                         </div>
//                       </div>
//                     </>
//                   )}
...
//               <h3><Badge>3</Badge> Tipografía y marco móvil</h3>
...
//                       onChange={(e)=> setProfile(p=>({ ...p, fontFamily: e.target.value }))}
...
//                   <ControlRow>
//                     Tamaño base
//                     <input type="range" min="14" max="20" value={profile.fontSize}
//                            onChange={e=>setProfile(p=>({ ...p, fontSize:Number(e.target.value) }))}/>
//                     <ValuePill>{profile.fontSize}px</ValuePill>
//                   </ControlRow>
//                 </div>

//                 <SectionHint>Vista móvil</SectionHint>

//                 <ControlRow>
//                   Ancho (px)
//                   <input type="range" min="320" max="430" value={profile.phoneWidth}
//                          onChange={(e)=> setProfile(p=>({ ...p, phoneWidth:Number(e.target.value) }))}/>
//                   <ValuePill>{profile.phoneWidth}</ValuePill>
//                 </ControlRow>

//                 <ControlRow>
//                   Padding
//                   <input type="range" min="10" max="36" value={profile.containerPadding}
//                          onChange={(e)=> setProfile(p=>({ ...p, containerPadding:Number(e.target.value) }))}/>
//                   <ValuePill>{profile.containerPadding}px</ValuePill>
//                 </ControlRow>

//                 <ControlRow>
//                   Offset superior
//                   <input type="range" min="0" max="140" value={profile.heroOffset}
//                          onChange={(e)=> setProfile(p=>({ ...p, heroOffset:Number(e.target.value) }))}/>
//                   <ValuePill>{profile.heroOffset}px</ValuePill>
//                 </ControlRow>

//                 <ControlRow>
//                   Espacio entre botones
//                   <input type="range" min="6" max="24" value={profile.linksGap}
//                          onChange={(e)=> setProfile(p=>({ ...p, linksGap:Number(e.target.value) }))}/>
...
//               <h3><Badge>4</Badge> Botones (estilo, colores, alineaciones)</h3>
...
//                       <button key={v} className={profile.btnVariant===v ? "active":""}
//                         onClick={()=> setProfile(p=>({ ...p, btnVariant:v }))}>
//                         {v}
//                       </button>
//                     ))}
//                   </div>
...
//                   <input type="checkbox" checked={profile.btnUseBrand}
//                          onChange={(e)=> setProfile(p=>({ ...p, btnUseBrand:e.target.checked }))}/>
...
//                   <div className="row">
//                     <label>BG</label>
//                     <input type="color" value={profile.btnBg} onChange={e=>setProfile(p=>({ ...p, btnBg:e.target.value }))}/>
//                     <label>Borde</label>
//                     <input type="color" value={profile.btnBorder} onChange={e=>setProfile(p=>({ ...p, btnBorder:e.target.value }))}/>
//                   </div>
//                 )}

//                 <div className="row">
//                   <label>Texto</label>
//                   <input type="color" value={profile.btnText} onChange={e=>setProfile(p=>({ ...p, btnText:e.target.value }))}/>
//                   <label>Grosor</label>
//                   <input type="range" min="1" max="6" value={profile.btnBorderWidth}
//                          onChange={(e)=> setProfile(p=>({ ...p, btnBorderWidth:Number(e.target.value) }))}/>
//                   <label>Radio</label>
//                   <input type="range" min="0" max="40" value={profile.btnRadius}
//                          onChange={(e)=> setProfile(p=>({ ...p, btnRadius:Number(e.target.value) }))}/>
//                   <label className="row" style={{ gap:6 }}>
//                     <input type="checkbox" checked={profile.btnPill}
//                            onChange={(e)=> setProfile(p=>({ ...p, btnPill:e.target.checked }))}/>
//                     Píldora (∞)
//                   </label>
//                   <label className="row" style={{ gap:6 }}>
//                     <input type="checkbox" checked={profile.btnShadow}
//                            onChange={(e)=> setProfile(p=>({ ...p, btnShadow:e.target.checked }))}/>
//                     Sombra
//                   </label>
//                 </div>

//                 <div className="row" style={{ width:"100%" }}>
//                   <span style={{ fontSize:14, color:"#6b7280" }}>Contenido del botón</span>
//                   <div className="seg">
//                     {["left","center","right"].map(v=>(
//                       <button key={v} className={profile.btnContentAlign===v ? "active":""}
//                         onClick={()=> setProfile(p=>({ ...p, btnContentAlign:v }))}>
//                         {v==="left"?"Izq.":v==="center"?"Centro":"Der."}
//                       </button>
//                     ))}
//                   </div>
...
//                   <div className="seg">
//                     {["left","right"].map(v=>(
//                       <button key={v} className={profile.btnIconSide===v ? "active":""}
//                         onClick={()=> setProfile(p=>({ ...p, btnIconSide:v }))}>
//                         {v==="left"?"Izq.":"Der."}
//                       </button>
//                     ))}
//                   </div>
...
//                 <div className="row" style={{ width:"100%" }}>
//                   <span style={{ fontSize:14, color:"#6b7280" }}>Ancho de botones</span>
//                   <div className="seg">
//                     {["stretch","center"].map(v=>(
//                       <button key={v} className={profile.btnAlign===v ? "active":""}
//                         onClick={()=> setProfile(p=>({ ...p, btnAlign:v }))}>
...
//                     <>
//                       <label>%</label>
//                       <input type="range" min="60" max="100" value={profile.btnWidth}
//                              onChange={(e)=> setProfile(p=>({ ...p, btnWidth:Number(e.target.value) }))}/>
//                     </>
//                   )}
//                 </div>
...
//                     <input type="file" accept="application/pdf" onChange={(e)=> e.target.files?.[0] && onPdfFile(e.target.files[0])}/>
//                   </div>
//                   {profile.pdfDataUrl && (
//                     <a href={profile.pdfDataUrl} target="_blank" rel="noreferrer" style={{ textDecoration:"none" }}>
//                       <Btn type="button">📄 Ver PDF: {profile.pdfName || "documento.pdf"}</Btn>
//                     </a>
//                   )}
//                 </div>
//               </StepBody>
...
//               <h3><Badge>5</Badge> Enlaces (URLs por red)</h3>
...
//                     return (
//                       <Card key={p.key}>
//                         <CardHeader $brand={p.brand} onClick={() => toggleOpen(p.key)} aria-expanded={opened}>
//                           <TitleSpan><Icon /> {p.name}</TitleSpan>
...
//                                   onChange={(e)=> setVisible(p.key, e.target.checked)}
//                                 />
//                                 Visible en tu perfil
//                               </label>
//                               <small style={{ color:"#6b7280" }}>Se admite https://, http://, mailto:, tel:</small>
//                             </Row>
//                           </CardBody>
//                         )}
//                       </Card>
...
//               <h3><Badge>6</Badge> Datos de contacto (para “Guardar contacto”)</h3>
//               <span>{openStep===6 ? "Ocultar" : "Editar"}</span>
//             </StepHeader>
//             {openStep===6 && (
//               <StepBody>
//                 <div className="row" style={{justifyContent:"space-between"}}>
//                   <Btn type="button" onClick={prefillFromLinks}>Rellenar desde enlaces</Btn>
...
//                   onChange={e=>setProfile(p=>({...p, contactFullName:e.target.value}))} />
//                 <div className="row" style={{width:"100%"}}>
//                   <TextInput placeholder="Empresa / Organización"
//                     value={profile.contactOrg}
//                     onChange={e=>setProfile(p=>({...p, contactOrg:e.target.value}))}/>
//                   <TextInput placeholder="Cargo"
//                     value={profile.contactTitle}
//                     onChange={e=>setProfile(p=>({...p, contactTitle:e.target.value}))}/>
//                 </div>

//                 <div className="row" style={{width:"100%"}}>
//                   <TextInput placeholder="Teléfono (solo números ej. +51999999999)"
...
//                   <TextInput placeholder="Website (https://...)"
//                     value={profile.contactWebsite}
//                     onChange={e=>setProfile(p=>({...p, contactWebsite:e.target.value}))}/>
//                 </div>

//                 <TextInput placeholder="Calle y número"
//                   value={profile.contactStreet}
//                   onChange={e=>setProfile(p=>({...p, contactStreet:e.target.value}))}/>
//                 <div className="row" style={{width:"100%"}}>
//                   <TextInput placeholder="Ciudad"
//                     value={profile.contactCity}
//                     onChange={e=>setProfile(p=>({...p, contactCity:e.target.value}))}/>
//                   <TextInput placeholder="Región/Provincia"
//                     value={profile.contactRegion}
//                     onChange={e=>setProfile(p=>({...p, contactRegion:e.target.value}))}/>
//                   <TextInput placeholder="C.P."
//                     value={profile.contactPostalCode}
//                     onChange={e=>setProfile(p=>({...p, contactPostalCode:e.target.value}))}/>
//                   <TextInput placeholder="País"
//                     value={profile.contactCountry}
//                     onChange={e=>setProfile(p=>({...p, contactCountry:e.target.value}))}/>
//                 </div>

//                 <TextArea placeholder="Nota (observaciones)"
//                   value={profile.contactNote}
//                   onChange={e=>setProfile(p=>({...p, contactNote:e.target.value}))}/>
//               </StepBody>
//             )}
//           </Step>
//         </EditorCol>
...
const Icons = {
  ...
  x: (p)=>(<svg viewBox="0 0 24 24" width="18" height="18" {...p}><path fill="currentColor" d="M4 3h5l3.6 5.1L17 3h3l-5.8 8.1L21 21h-5l-4-5.7L7 21H4l6.3-9L4 3Z"/></svg>),
  telegram: (p)=>(<svg viewBox="0 0 24 24" width="18" height="18" {...p}><path fill="currentColor" d="M9.1 13.1 17.8 7 6.7 11.4l2.4 1.7v4.1l3.1-2.8 3.5 2.5c.4.2.8 0 .9-.5l2.2-10c.1-.6-.4-1.1-1-1L3.6 9.4c-.8.3-.7 1.5.1 1.7l5.4 1.4Z"/></svg>),
  pdf: (p)=>(<svg viewBox="0 0 24 24" width="18" height="18" {...p}><path fill="currentColor" d="M14 2H6a2 2 0 0 0-2 2v16c0 1.1.9 2 2 2h12a2 2 0 0 0 2-2V8l-6-6Zm1 7V3.5L19.5 9H15ZM7 13h7v2H7v-2Zm0 4h10v2H7v-2Zm0-8h5v2H7V9Z"/></svg>),
  custom: (p)=>(<svg viewBox="0 0 24 24" width="18" height="18" {...p}><path fill="currentColor" d="M12 2 9.5 8H3l5.2 3.8L6.6 18 12 14.4 17.4 18l-1.6-6.2L21 8h-6.5L12 2Z"/></svg>)
};
...
const Swatch = styled.span`
  width:22px; height:22px; border-radius:7px; border:1px solid #e5e7eb; display:inline-block;
  background:${p=>p.$bg};
`;
...
const StepHeader = styled.button`
  all:unset; display:flex; align-items:center; justify-content:space-between; gap:12px;
  width:100%; padding:14px 16px; cursor:pointer; background:#f8fafc; border-bottom:1px solid #eef2f7;
  h3{margin:0; display:flex; align-items:center; gap:10px; font-size:16px}
`;
const Badge = styled.span`
  display:inline-flex; align-items:center; justify-content:center;
...
const ControlRow = styled.label`
  ...
  gap:10px; align-items:center; font-size:14px; color:#111827;
`;
const ValuePill = styled.span`
  justify-self:end; min-width:52px; font-variant-numeric: tabular-nums;
  background:#f1f5f9; border:1px solid #e5e7eb; padding:6px 8px; border-radius:10px; text-align:center; color:#0f172a;
`;
const SectionHint = styled.div` font-size:12px; color:#64748b; margin-top:4px; `;
...
const CardHeader = styled.button`
  all: unset; display:flex; align-items:center; justify-content:space-between; gap:10px;
  padding:14px 16px; width:100%; cursor:pointer; font-weight:600; color:#fff;
  background: ${({$brand}) => `linear-gradient(135deg, ${$brand} , ${$brand}cc)`};
`;
const TitleSpan = styled.span`display:inline-flex; align-items:center; gap:10px; font-size:16px;`;
const Toggle = styled.span`
  background: rgba(255,255,255,.18); padding: 6px 12px; border-radius: 999px; font-size:12px; margin-right: 24px;
`;
const CardBody = styled.div` padding:14px 14px 16px; border-top: 1px solid #e5e7eb; `;
const Row = styled.div`
  display:flex; gap:8px; align-items:center; margin-bottom:10px;
...
/* ====== Reordenar ====== */
const ReorderBar = styled.div`display:flex; gap:8px; align-items:center; margin-bottom:8px;`;
const ReList = styled.ul`list-style:none; margin:0; padding:0; display:grid; gap:8px;`;
const ReItem = styled.li`
  display:flex; align-items:center; gap:10px; padding:10px 12px;
  border:1px solid #e5e7eb; border-radius:12px; background:#fff;
`;
const DragHandle = styled.span`cursor:grab; user-select:none; font-size:18px; opacity:.6;`;
...
const STYLE_PRESETS = [
  {
    key: "minimal",
    name: "Minimal",
    demo: "linear-gradient(135deg,#ffffff,#f3f4f6)",
    patch: {
      bgMode: "solid",
      bgColor: "#0b1220",
      textColor: "#ffffff",
  ...
  },
  {
    key: "neon",
    name: "Neon",
    demo: "linear-gradient(135deg,#111827,#0ea5e9)",
    patch: {
      bgMode: "gradient",
      bgColor: "#0f172a",
      bgColor2: "#0ea5e9",
  ...
  },
  {
    key: "pastel",
    name: "Pastel",
    demo: "linear-gradient(135deg,#fde68a,#fbcfe8)",
    patch: {
      bgMode: "gradient",
      bgColor: "#fde68a",
      bgColor2: "#fbcfe8",
  ...
  },
  {
    key: "vidrio",
    name: "Vidrio",
    demo: "linear-gradient(135deg,#0b1220,#312e81)",
    patch: {
      bgMode: "image",
      bgImageDataUrl: "",
      overlayOpacity: 0.35,
...
export const Menu = () => {
  ...
  useEffect(() => {
    ...
    if (raw) {
      try {
        const saved = JSON.parse(raw);
        const savedLinks = Array.isArray(saved) ? saved : (Array.isArray(saved?.links) ? saved.links : []);
        // asegurar que existan TODAS las plataformas (incluido PDF)
        const merged = PLATFORMS.map(p => savedLinks.find(s => s.key === p.key) || { key: p.key, url: "", visible: true, open: false });
        setItems(merged);
        if (!Array.isArray(saved) && saved?.profile) setProfile(prev => ({ ...prev, ...saved.profile }));
      } catch (e) {
  ...
  // helpers
  const byKey = (k) => items.find(i => i.key === k);
/*   const idxByKey = (k) => items.findIndex(i => i.key === k); */
  const toggleOpen = (k) => setItems(prev => {
    const isOpen = prev.find(i => i.key === k)?.open;
    return prev.map(i => i.key === k ? { ...i, open: !isOpen } : { ...i, open: false });
  });
  ...
  const activeCount = visibleLinks.length;

  const validateOne = (k) => {
    const i = byKey(k);
    if (!i) return;
    const ok = i.url ? isValidUrl(i.url) : true;
    setErrors(e => ({ ...e, [k]: ok ? "" : "URL inválida (https://, mailto:, tel:, data:)" }));
  };

  // assets
  const onAvatarFile = (file) => { const r = new FileReader(); r.onload = () => setProfile(p => ({ ...p, avatarDataUrl: r.result })); r.readAsDataURL(file); };
  const onBgImageFile = (file) => { const r = new FileReader(); r.onload = () => setProfile(p => ({ ...p, bgImageDataUrl: r.result, bgMode: "image" })); r.readAsDataURL(file); };
  const onPdfFile = (file) => { const r = new FileReader(); r.onload = () => {
      setProfile(p => ({ ...p, pdfDataUrl: r.result, pdfName: file.name }));
      setUrl("pdf", r.result); // enlazar PDF como link
    };
  ...
  const prefillFromLinks = () => {
    ...
    const email = (byKey("email")?.url || "").replace(/^mailto:/,"");
    const website = byKey("website")?.url || "";
    setProfile(p => ({ ...p, contactPhone: phone || p.contactPhone, contactEmail: email || p.contactEmail, contactWebsite: website || p.contactWebsite }));
  };
  ...
  const surprise = () => {
    const idx = Math.floor(Math.random()*STYLE_PRESETS.length);
    applyPreset(STYLE_PRESETS[idx].key);
  };
  ...
  const onDropOver = (overKey) => {
    if (!dragKey || dragKey === overKey) return;
    setItems(prev => {
      const arr = [...prev];
      const from = arr.findIndex(i => i.key === dragKey);
      const to = arr.findIndex(i => i.key === overKey);
  ...
  return (
    <Wrap>
      ...
      <PresetsBar>
        ...
        {STYLE_PRESETS.map(p=>(
          <PresetBtn key={p.key} onClick={()=>applyPreset(p.key)} title={p.name}>
      ...
      <Grid>
        ...
        <PhoneCol>
          <Phone $w={profile.phoneWidth}>
            ...
            >
              ...
              <Heading $align={profile.align}>
                <h2>{profile.title || "Tu nombre o marca"}</h2>
        ...
        <EditorCol>
          ...
          <Step>
            <StepHeader onClick={()=> setOpenStep(s=> s===5 ? 0 : 5)}>
              <h3><Badge>5</Badge> Enlaces ({activeCount})</h3>
              <span>{openStep===5 ? "Ocultar" : "Editar"}</span>
            </StepHeader>
            {openStep===5 && (
              <StepBody>
                <ReorderBar>
                  <Btn type="button" onClick={()=> setReorder(r=>!r)}>{reorder ? "Terminar reorden" : "Reordenar enlaces"}</Btn>
                  <small style={{color:"#64748b"}}>Arrastra con ☰ para cambiar el orden.</small>
                </ReorderBar>

                {reorder ? (
                  <ReList>
                    {items.map(it=>{
                      const Icon = Icons[it.key] || Icons.custom;
                      return (
                        <ReItem key={`re-${it.key}`}
                                draggable
                                onDragStart={()=> onDragStart(it.key)}
                                onDragOver={(e)=> e.preventDefault()}
                                onDrop={()=> onDropOver(it.key)}>
                          <DragHandle title="Arrastrar">☰</DragHandle>
                          <Icon />
                          <strong style={{flex:1}}>{PLATFORMS.find(p=>p.key===it.key)?.name || it.key}</strong>
                          <label style={{display:"flex", alignItems:"center", gap:6}}>
                            <input type="checkbox" checked={it.visible ?? true}
                                   onChange={(e)=> setVisible(it.key, e.target.checked)} />
                            Visible
                          </label>
                        </ReItem>
                      );
                          ...
                          {opened && (
                            <CardBody>
                              ...
                              {p.key === "pdf" ? (
                                <>
                                  <Row>
                                    ...
                                    {profile.pdfName && <small style={{color:"#64748b"}}>📄 {profile.pdfName}</small>}
                                    {profile.pdfDataUrl && (
                                      <Btn type="button" onClick={()=>{
                                        setProfile(pr=>({...pr, pdfDataUrl:"", pdfName:""}));
                                        setUrl("pdf", "");
                                      }}>Quitar</Btn>
                                    )}
                                  </Row>
                              ...
                              <Row>
                                ...
                                <small style={{ color:"#6b7280" }}>Se admite https://, http://, mailto:, tel:, data:, blob:</small>
                              </Row>
                            </CardBody>
                          )}
                        </Card>
